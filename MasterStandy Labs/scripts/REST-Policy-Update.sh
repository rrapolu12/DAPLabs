...
#	- This demonstration will showcase how to update DAP policy
#
...
echo " "
echo "Now enter the Policy Information to Update:"
echo " "
...
# Connect to the Conjur/DAP REST API to update policy"
echo "---- Updating Policy ----"
echo "Policy Branch: $policy"
echo "Policy File: $file"
echo "----------------------------------------"
curl -s -k --request PATCH --header "Authorization: Token token=\"$token\"" --data "$(< $file)" "https://$dapURL/policies/$account/policy/$policy"
echo " "
